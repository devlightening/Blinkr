...
- `App.tsx`: navigasyon kabugu. Kutuphanesiz `activeTab` (`chat | map | nearby | profile`); `MapScreen` her zaman monte kalir (viewport, katman ve marker'lar sekme degisince kaybolmaz), Sohbet, Yakinda ve Profil onun ustunde tam ekran katman olarak acilir. Tek alt bar `BlinkrBottomBar` (ekranin en altina yapisik, kenardan kenara, altindaki guvenli alani kendisi tasir; Harita | Kesfet | + | Sohbet | Profil; ortadaki "+" sekme degil eylemdir: dokunma kamerayi, basili tutma yalniz yazili sinyal composer'ini acar - sinyal-mvp-plan P5.1); sheet/composer/acik konusma/avatar secici varken bar gizlenir (native'de kardes zIndex sirasi yuzunden bar acik sheet'in ustune biner). Sekmeler arasinda Instagram gibi kaydirilir (`TabPager`, yerel `react-native-pager-view`; sira Harita | Kesfet | Sohbet | Profil): haritanin sayfasi saydamdir, harita altta monte kalir ve harita sekmesindeyken sayfalayici dokunusu tamamen birakir (haritayi kaydirmak sekme degistirmez); bir sayfada katman acikken kaydirma kapalidir; alt cubuga dokunmak animasyonsuz gecer; yalniz aktif sekme ve komsulari cizilir. Paylas ve "kayitli yeri ac" tek seferlik istektir (`shareRequested`/`focusPlace` + `on...Handled`), Android geri tusu sekmeyi haritaya dondurur.
...
- V2-4 (D-027): begeni dugmesi `ui/ReactionButton` (dokun = ❤️/geri al, uzun bas = 6 emoji); kullanici metni `ui/RichText` ile (yalniz sunucunun cozdugu @ad profile, #etiket Kesfet etiket akisina gider; kural `richText.ts`); yorum ve composer aciklamasinda "@" yazinca `ui/MentionSuggestions`; yorumlarda kalp + sayi.
- V2-7 (D-029): paylas dugmesi `chat/ShareToChatSheet` menusunu acar (baglantiyi kopyala `blinkr://posts/{id}`, diger uygulamalar = sistem paylasimi, metin `deepLinks.shareMessage`: ozet + yer + baglanti, paylasan adi yok; sohbette arkadasa gonder). Bildirimler (Etkinlik) "Yeni / Bugun / Bu hafta / Daha once"; ayni gonderiye tepkiler ve ayni hikayeye begeniler 1 saat icinde tek satir, satirda iki yuz (`rowAvatars`).
- V2-6: `feed/FeedCard` Instagram duzeni (ust bilgi, kenardan kenara `MediaCarousel`, cift dokunma = ❤️ ekler asla geri almaz, eylemler, rozet, ad onde baslik + "devami", "N yorumun tumunu gor"); Kesfet listesi yalniz en gorunur kartin videosunu oynatir ve >= 1 sn gorulen karti `POST /api/posts/views` ile sayar (10 sn toplu, kendi sinyalin sayilmaz); "#" sekmesi `feed/HashtagSearch`; profil izgarasi videoyu kucuk resmi + ▶ ile gosterir.
- `feed/DiscoverScreen` ("Kesfet" sekmesi; hikaye seridi akisin basindadir ve gonderilerle birlikte kayar; gonderide cift dokunma (fotograf, video ve yazi) ❤️ ekler ve `ui/ReactionBurst` emojiye ozgu animasyonu oynatir, emoji secince de ayni animasyon; sinyal-mvp-plan Faz 7; V2-4: `#etiket` modunda sekmeler yerine etiket akisi, geri ile doner, harita kartindan da `hashtagRequest` ile acilir): Yakinimda (`GET /api/discover/nearby`, tazelik > yakinlik > log etkilesim, kisi basina sayfada en fazla 2, kaba mesafe, anonimde yazar yok), Takip (`GET /api/discover/following`, son 7 gun, anonim yok) ve Yerler (asagidaki `NearbyScreen`, `embedded`). `feed/FeedCard`: begeni yerinde (iyimser), yorumlar `SignalThreadPanel` sheet'inde, yazar profili `UserProfileSheet`, yere bagli sinyalde "Haritada goster". Sonsuz akis yok (en fazla 10 sayfa), yenileme hatasinda son liste kalir.
...
- `SettingsScreen` (Profil ustundeki disli): hesap bilgisi, Sifreyi degistir (`account/ChangePasswordView`), Izinler (konum/kamera/fotograf telefonun bildirdigi durum, dokununca sistem ayarlari), Arama gecmisini temizle, Verilerimi iste (`account/DataRequestView`), Hesabi sil (`account/DeleteAccountView`), Hakkinda altinda Topluluk kurallari / Kullanim sartlari / Gizlilik politikasi (`LegalDocView`, metinler `legalContent.ts`, taslak notlu), Engellenen kisiler (engeli kaldir), gizlilik ozeti, surum ve OpenStreetMap atfi (ODbL; yer verisi lisansi geregi), cikis. Yalniz gercek bilgi vardir; hicbir sey yapmayan anahtar yoktur. `ReportPanel`: sheet icinde rapor formu (sebep secimi, istege bagli not); profil ve sinyal detayi ayni sheet'te icerigi degistirir, ikinci bir sheet acmaz. Sohbette ust cubuktaki ad/avatar profili acar; oradan engellenen kisinin sohbeti listeden kalkar.
...
- `POST /api/users/me/password` (`{ currentPassword, newPassword, refreshToken }`; 400 `WRONG_PASSWORD`/`PASSWORD_TOO_SHORT`/`PASSWORD_TOO_LONG`/`PASSWORD_UNCHANGED`, 10 yanlis denemede 15 dk kilit 429 `TOO_MANY_ATTEMPTS`; diger tum oturumlar kapanir, bu cihaz acik kalir; BLK-PASSWORD-01)
...
- `POST /api/posts/{id}/reactions` (V2-4, D-027; `{ reaction }`, set ❤️ 🔥 😂 😮 😢 👏, kisi basina bir, ayni emoji/null geri alir; `{ reaction, counts }`; 400 `INVALID_REACTION`/`CANNOT_LIKE_OWN`). `GET /api/posts/{id}` ve Kesfet ogeleri `reactionCounts`, `myReaction`, `mentions` (`[{ userId, userName }]`), detay `hashtags` da tasir.
- `POST /api/posts/{id}/comments/{commentId}/like` (V2-4; toggle, `{ liked, likeCount }`; kendi yorumu serbest). Yorumlarda `likeCount`, `likedByMe`, `mentions`; begenenler listelenmez.
- Mention: gonderi ve yorum metnindeki @adlar sunucuda cozulur (istemci id gondermez; IdentityService `POST /api/users/resolve`, engelli/silinmis duser); 10'dan fazla ad 400 `TOO_MANY_MENTIONS`; bildirim tipi `Mentioned`.
- `GET /api/posts-read/bounds`
- `GET /api/posts-read/nearby`
- `GET /api/posts-read/author/{id}?page&pageSize` (sayfa numarasi en fazla 1000; `X-Total-Count`; V2-6: her ogede `media[{ url, thumbnailUrl, type }]`): yazarin paylasimlari. Anonim (`AnonymousMap`) paylasimlar yalnizca yazarin kendisine doner ve o yanit `Cache-Control: private, no-store`'dur; baskalarina, giris yapmamislara ve herkese acik `?authorId=` listesine asla donmez (anayasa 10.3).
- `POST /api/posts/place-presence`

### Map

- `GET /api/map/bounds` (giris yapmis kisiye engelin iki yanindaki kisilerin koordinat sinyalleri gosterilmez, anonimler haric; PlaceService 8 sn icinde cevap vermezse yerler bos ve `X-Blinkr-Places: unavailable`, uygulama onceki yerleri korur)
- `GET /api/map/nearby`
...
- `GET /api/notifications` (V2-7: gruplu satirlarda `actorCount` ve en yeni iki kisi `actorIds`; tip `Mentioned` ve `StoryLiked` dahil)
...
- Engel etkileri: arama sonucunda ve `GET /api/users/{id}` ile iki taraf birbirini goremez (engellenen icin 404; engelleyen icin `relation: "blocked"` ve bos profil); arkadaslik istegi 403 `REQUEST_NOT_ALLOWED`; sohbet/mesaj/snap 403 `CHAT_FORBIDDEN`. Yorum, yanit, tepki, begeni ve yorum begenisi: sinyalin sahibi (ve yanitlanan/begenilen yorumun yazari) ile arada engel varsa 404 `NOT_FOUND` (engel belli edilmez); engel okunamazsa 503 `INTERACTION_UNAVAILABLE` (`InteractionGuard`). Anonim sinyalin yazari bu denetime girmez, yoksa ret yazari ele verirdi. Yorum listesi (`GET /api/posts/{id}/comments`) giris yapmis okuyucuya engelin iki yanindaki kisilerin yorum ve yanitlarini gostermez; engel okunamazsa 503 `COMMENTS_UNAVAILABLE`.
