...
$originB = @{ lat = [double]$place.latitude + 0.0027; lon = [double]$place.longitude }
...
Assert-True ([Math]::Abs($distanceB - $distanceA) -gt 100) "Distance appears stale; it did not change meaningfully across origins."

$farOnlyPrimary = @($nearA | Where-Object { [double]$_.distanceMeters -le 600 })
$extended = @($nearA | Where-Object { [double]$_.distanceMeters -gt 600 })
